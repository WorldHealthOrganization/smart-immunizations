...
Feature: IMMZ.D2.DT.Hepatitis A.Live attenuated HAV 1 dose Decision Table Live attenuated hepatitis A virus (HAV), 1 dose schedule
  ...
  Scenario: Client's age is less than 18 months: Client is not due for Hepatitis A vaccination
    ...
    And url urlBase
    And path 'PlanDefinition/IMMZD2DTHepatitisALiveAttenuatedHAV1Dose/$apply'
    And applyParams.parameter[0].valueString = 'Patient/HepatitisA30.1'
    And request applyParams
    When method post
    Then status 200
    And resultWithoutMedication.contained[0].subject.reference = 'Patient/HepatitisA30.1'
    And resultWithoutMedication.contained[1].payload.contentString = "Should not vaccinate client with Hepatitis A dose as client's age is less than 18 months. \nCheck for any other vaccines due and inform the caregiver of when to come back for the first dose."
  ...
  Scenario: No hepatitis A primary series dose was administered, Client's age is more than or equal to 18 months, and Live vaccine was administered in the last 4 weeks: Client is not due for Hepatitis A vaccination
    ...
    When method post
    Then status 200
    And resultWithoutMedication.contained[0].subject.reference = 'Patient/HepatitisA31.3'
    And resultWithoutMedication.contained[1].payload.contentString = "Should not vaccinate client with Hepatitis A dose as live vaccine was administered in the last 4 weeks. \nCheck for any other vaccines due and inform the caregiver of when to come back for the first dose."
    And match response contains deep resultWithoutMedication
    And match response.contained == '#[2]'
  ...
  Scenario: No hepatitis A primary series dose was administered, Client's age is more than or equal to 18 months, and No live vaccine was administered in the last 4 weeks: Client is due for Hepatitis A vaccination
    ...
    When method post
    Then status 200
    And resultWithMedication.contained[0].subject.reference = 'Patient/HepatitisA32.3'
    And resultWithMedication.contained[1].payload.contentString = "Should vaccinate client with hepatitis A dose as no Hepatitis A dose was administered, client is within appropriate age range and no live vaccine was administered in the last 4 weeks. \nCheck for contraindications."
  ...
  Scenario: One hepatitis A primary series dose was administered: Hepatitis A immunization schedule is complete
    ...
    When method post
    Then status 200
    And resultWithoutMedication.contained[0].subject.reference = 'Patient/HepatitisA33.1'
