...
* library = Canonical(DT.IMMZ.D2.DT.DTP.Pregnancy)
* extension[+]
...
Tetanus vaccines: WHO position paper (February 2017) (7)"""
  ...
  * action[output]
  * title = If tetanus vaccination is started during adolescence or adulthood, including pregnancy, a total of only 5 appropriately spaced doses are required to obtain lifelong protection. 
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = Pregnant women who received 4 tetanus-containing vaccine doses during childhood or pre-adulthood need only 1 booster dose, which should be given at the first opportunity. To provide lifelong protection, a sixth dose would be needed at least 1 year after the fifth dose, or duing the next pregnancy. 
   * description = {description}
