...
* library = Canonical(DT.IMMZ.D2.DT.HPV.2doses)
* extension[+]
...
* relatedArtifact[+]
  ...
  * action[output]
  * title = The current evidence supports the recommendation that a 2-dose schedule be used in the primary target group from 9 years of age and for all older age groups for which HPV vaccines are licensed. 
 ...
 * dynamicValue[+]
  ...
  * action[output]
  * title = The WHO-recommended primary target population for HPV vaccination is girls aged 9–14 years. Prevention of cervical cancer is best achieved through the immunization of girls before they become sexually active. 
   * description = {description}
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = The minimum interval between first and second dose is 6 months. A 12-month schedule results in higher geometric mean titres (GMTs) and is suggested for programmatic and efficiency reasons. 
   * description = {description}
   * definitionCanonical = Canonical(DTO.ClientisnotdueforHPVvaccination)
  * dynamicValue[+]
    * path = "status"
