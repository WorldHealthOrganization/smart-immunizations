...
* library = Canonical(DT.IMMZ.D2.DT.HepatitisA.InactivatedHAV2doses)
* extension[+]
...
* relatedArtifact[+]
  ...
  * action[output]
  * title = For children, inactivated hepatitis A-containing vaccines can be given as a single-dose or 2-dose schedule, and administered intramuscularly.  
 ...
 * dynamicValue[+]
  ...
  * action[output]
  * title = With a 2-dose schedule, the first dose should be given from ≥ 12 months of age. 
   * description = {description}
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = The interval between doses is flexible, from 6 months to 4–5 years or more, but is usually 6–18 months. 
   * description = {description}
   * definitionCanonical = Canonical(DTO.ClientisnotdueforHepatitisAvaccination)
  * dynamicValue[+]
    * path = "status"
