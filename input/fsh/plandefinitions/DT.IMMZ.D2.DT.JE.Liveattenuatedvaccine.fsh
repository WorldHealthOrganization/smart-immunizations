...
* library = Canonical(DT.IMMZ.D2.DT.JE.Liveattenuatedvaccine)
* extension[+]
...
* relatedArtifact[+]
  ...
  * action[output]
  * title = Live attenuated vaccine: Single dose administered at ≥ 8 months of age. 
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = Live attenuated vaccine: Single dose administered at ≥8 months of age. As a general rule, any live vaccine may be given either simultaneously or at an interval of 4 weeks. 
   * description = {description}
   * definitionCanonical = Canonical(DTO.ClientisnotdueforJEvaccination)
  * dynamicValue[+]
    * path = "status"
