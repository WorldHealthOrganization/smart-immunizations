...
* library = Canonical(DT.IMMZ.D2.DT.Pneumococcal.2doseswithboosterdose)
* extension[+]
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = If the 2p+1 schedule is selected, an interval of ≥8 weeks is recommended between the 2 primary doses. 
   * description = {description}
   * definitionCanonical = Canonical(DTO.Clientisnotdueforpneumococcalvaccination)
  * dynamicValue[+]
    * path = "status"
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = Catch-up vaccination can be done with a single dose of vaccine for children aged 24 months and older. Pneumococcal immunization schedule is complete. 
   * description = {description}
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = For the 2p+1 schedule, the booster dose should be given at 9–18 months of age, according to programmatic considerations; there is no defined minimum or maximum interval between the primary series and the booster dose. 
   * description = {description}
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = HIV-positive infants and preterm neonates who have received their 3 primary vaccine doses before 12 months of age may benefit from a booster dose in the second year of life. There is no defined minimum or maximum interval between the primary series 
 ...
 * dynamicValue[+]
   ...
   * expression
     ...
     * expression = "Code { system: 'http://hl7.org/fhir/request-priority', code: 'routine' }"
  * action[output]
  * title = WHO does not currently have recommendations on the use of pneumococcal vaccine in individuals aged over 5 years.
