# IMMZD2DTMalaria ValueSet for Decision Table - WHO Immunization Implementation Guide v0.2.0

* [**Table of Contents**](toc.md)
* [**Indices**](indices.md)
* [**Artifact Index**](artifacts.md)
* **IMMZD2DTMalaria ValueSet for Decision Table**

## ValueSet: IMMZD2DTMalaria ValueSet for Decision Table 

| | |
| :--- | :--- |
| *Official URL*:http://smart.who.int/immunizations/ValueSet/IMMZD2DTMalariaVS | *Version*:0.2.0 |
| Active as of 2026-07-20 | *Computable Name*:IMMZD2DTMalariaVS |

 
ValueSet IMMZD2DTMalaria for IMMZ.D2.DT.Malaria. Business rule: Determine if the client is due for a malaria vaccination according to the national immunization schedule Trigger: IMMZ.D2 Determine required vaccination(s) if any Table: 4-dose schedule (The RTS,S/AS01 malaria vaccine should be used for the prevention of Plasmodium falciparum malaria in children living in regions with moderate to high malaria transmission, as defined by WHO) 

 **References** 

This value set is not used here; it may be used elsewhere (e.g. specifications and/or implementations that use this content)

### Logical Definition (CLD)

 

### Expansion

-------

 Explanation of the columns that may appear on this page: 

| | |
| :--- | :--- |
| Level | A few code lists that FHIR defines are hierarchical - each code is assigned a level. In this scheme, some codes are under other codes, and imply that the code they are under also applies |
| System | The source of the definition of the code (when the value set draws in codes defined elsewhere) |
| Code | The code (used as the code in the resource instance) |
| Display | The display (used in the*display*element of a[Coding](http://hl7.org/fhir/R4/datatypes.html#Coding)). If there is no display, implementers should not simply display the code, but map the concept into their application |
| Definition | An explanation of the meaning of the concept |
| Comments | Additional notes about how to use the code |

## API Information

##### IMMZD2DTMalaria ValueSet for Decision Table Schema API

JSON Schema for IMMZD2DTMalaria ValueSet for Decision Table ValueSet codes. Generated from FHIR expansions using IRI format.

**Version:** 1.0.0

## Endpoints

### GET /ValueSet-IMMZD2DTMalariaVS.schema.json

#### JSON Schema definition for the enumeration ValueSet-IMMZD2DTMalariaVS

This endpoint serves the JSON Schema definition for the enumeration ValueSet-IMMZD2DTMalariaVS.

## Schema Definition

### ValueSet-IMMZD2DTMalariaVS

**Description:** JSON Schema for IMMZD2DTMalaria ValueSet for Decision Table ValueSet codes. Generated from FHIR expansions using IRI format.

**Type:** string

**This documentation is automatically generated from the OpenAPI specification.**



## Resource Content

```json
{
  "resourceType" : "ValueSet",
  "id" : "IMMZD2DTMalariaVS",
  "url" : "http://smart.who.int/immunizations/ValueSet/IMMZD2DTMalariaVS",
  "version" : "0.2.0",
  "name" : "IMMZD2DTMalariaVS",
  "title" : "IMMZD2DTMalaria ValueSet for Decision Table",
  "status" : "active",
  "date" : "2026-07-20T11:34:37+00:00",
  "publisher" : "WHO",
  "contact" : [{
    "name" : "WHO",
    "telecom" : [{
      "system" : "url",
      "value" : "http://who.int"
    }]
  }],
  "description" : "ValueSet IMMZD2DTMalaria for IMMZ.D2.DT.Malaria.\nBusiness rule: Determine if the client is due for a malaria vaccination according to the national immunization schedule\nTrigger: IMMZ.D2 Determine required vaccination(s) if any\nTable: 4-dose schedule (The RTS,S/AS01 malaria vaccine should be used for the prevention of Plasmodium falciparum malaria in children living in regions with moderate to high malaria transmission, as defined by WHO)",
  "compose" : {
    "include" : [{
      "system" : "http://smart.who.int/immunizations/CodeSystem/IMMZDAK",
      "concept" : [{
        "code" : "Client's age is less than 5 months-41",
        "display" : "Client's age is less than 5 months"
      },
      {
        "code" : "Client is not due for malaria vaccination-48",
        "display" : "Client is not due for malaria vaccination"
      },
      {
        "code" : "No malaria primary series doses were administered-116",
        "display" : "No malaria primary series doses were administered"
      },
      {
        "code" : "Client's age is more than or equal to 5 months-41",
        "display" : "Client's age is more than or equal to 5 months"
      },
      {
        "code" : "Client is due for malaria vaccination-44",
        "display" : "Client is due for malaria vaccination"
      },
      {
        "code" : "One malaria primary series dose was administered-116",
        "display" : "One malaria primary series dose was administered"
      },
      {
        "code" : "The latest malaria dose was administered less than 4 weeks ago-99",
        "display" : "The latest malaria dose was administered less than 4 weeks ago"
      },
      {
        "code" : "The latest malaria dose was administered more than 4 weeks ago-106",
        "display" : "The latest malaria dose was administered more than 4 weeks ago"
      },
      {
        "code" : "Two malaria primary series doses were administered-116",
        "display" : "Two malaria primary series doses were administered"
      },
      {
        "code" : "Three malaria primary series doses were administered-116",
        "display" : "Three malaria primary series doses were administered"
      },
      {
        "code" : "Four malaria primary series doses were administered-116",
        "display" : "Four malaria primary series doses were administered"
      },
      {
        "code" : "Malaria immunization schedule is complete-144",
        "display" : "Malaria immunization schedule is complete"
      }]
    }]
  }
}

```
